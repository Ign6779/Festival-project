<!-- font families  -->
<link href='https://fonts.googleapis.com/css?family=Source+Sans+Pro' rel='stylesheet' type='text/css'>
<link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/css?family=Corben" />
<link href='https://fonts.googleapis.com/css?family=Mandali' rel='stylesheet'>


<ol>
    <li><a href="festival/overview">GOTO overview</a></li>
    <li><a href="festival/dance">GOTO dance</a></li>
    <li><a href="festival/history">GOTO history</a></li>
    <li><a href="festival/jazz">GOTO jazz</a></li>
    <li><a href="festival/kids">GOTO kids</a></li>
    <li><a href="festival/yummy">GOTO yummy</a></li>
</ol>

<body>
<!-- Header of the homepage -->
<section class="homepage-header">
<h1>Welcome to Haarlem</h1>
<h3>Welcome to the city of endless possibilities: A charming historic district, fascinating museums, unique shops, a wide variety of dining options, and a lovely beach, 
<br></h3>
<h2>Haarlem really has it all.</h2>
<br><a class="btn-view-more" href="festival/overview">View Festival Activities</a>
</section>

<div class="homepage-history container-fluid">
    <div>
    <h2>Haarlem's History</h2>
    </div>
    <div>
    <h3>Haarlem has been an important Dutch city since 1245, functioning initially as a shipping sea port. 
    The majority of Haarlem's old gorgeous buildings have stayed intact and are what make the atmosphere in Haarlem truly amazing. Well known for its beer making, Haarlem is home to the wonderful Jopenkerk and windmill de Adriaan from 1779.</h3>
    </div>
</div>

<section class="homepage-food">
    <div>
    <h3>No matter when, dining in Haarlem is guaranteed to be an incredible experience.
Not only does Haarlem have some of the best restaurants with quality food, the atmosphere of the city is what makes it all the more special. </h3>
    </div>
    <div>
        <h2>For Foodies!</h2>
    </div>
    <div>
    </div>
</section>
<div class = "grid">
<section class="homepage-music">
<h2>Music</h2>
    <h3>The city of Haarlem comes alive with music in July. The days are longer and warmer, making it the ideal time to go see both well-known and unknown Dutch musicians play for the delight of the crowd.</h3>
</section>

<section class="homepage-theatre">
    <h2>Theatre</h2>
    <h3>When it comes to entertainment, Haarlem is perfect. Theatres such as :Het Kennenmer Theatre, and Philharmonie will guarantee you won't leave Haarlem disappointed. 
Don't miss out on the beautiful dramas and the high quality productions!</h3>
</section>

<section class="homepage-museums">
    <h2>Museums</h2>
    <h3>If you want to see some amazing historical artefacts and you want to learn their history, then come to Haarlem’s museums!<br>Some great choices include:
<br>&#x2022;The corrie ten boom house
<br>&#x2022;Teylers Museum
<br>&#x2022;Frans hals museum
<br>&#x2022;Museum van de geest</h3>
</section>
</div>


<section class="homepage-festival">
    <h2>Check out the festival here!</h2>
    <a class="festival-button" href="festival/overview">See festival events</a>
</section>

<section class="homepage-mobile">
    <h2>Visit us on mobile.</h2>
    <h3> Scan the QR code with your phone.</h3>
</section>




</body>
<?php

?>

<style>
body{
    width: 100%;
    font-family: "Source Sans Pro";
    background-color: #FFF8F0;
}
.h1{
    font-family: "Corben";
}
.h2{
    font-family: "Mandali";
}
.h3{
    font-family: "Source Sans Pro";
}
.homepage-history{
    background-color: #92140C;
    width: 100%;
    color: #FFFEFE;
    font-family: "Source Sans Pro";
    padding-bottom: 10em;
    padding-top: 15px;
    display: grid;
    grid-template-columns: 66% 33%;

}
.homepage-food{
    font-family: "Source Sans Pro";
    background: #FFF8F0;
    display: grid;
    grid-template-columns: 33% 33% 33%;
}

.homepage-music{
    background-color: #9EADEA;
    padding-left: 10px;
}
.homepage-theatre{
    background-color: #111D4A;
    color: #FFFEFE;
    padding: 10px;
}
.homepage-museums{
    background-color: #9EADEA;
    padding: 10px;
}
.grid{
    display: grid;
  grid-template-columns: 33% 33% 33%;
  grid-gap: 10px;
}
.homepage-header{
    text-align: center;
    padding-bottom: 5em;
}
.homepage-festival{
    text-align: center;
}
.festival-button{
    background-color: #7A86B6;
    border-radius: 25px;
  border: none;
  color: white;
  padding: 15px 35px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  box-shadow: 5px 10px #888888;
}
/* add changing color on hover */
.festival-button:hover{
    background-color: #ffffff;
    border-radius: 25px;
  border: none;
  color: #7A86B6;
  padding: 15px 35px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  box-shadow: 5px 10px #888888;
}
.btn-view-more{
    background-color: #92140C;
    border-radius: 25px;
  border: none;
  color: white;
  padding: 15px 35px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  box-shadow: 5px 10px #888888;
}
.btn-view-more:hover{
    background-color: white;
    border-radius: 25px;
  border: none;
  color: #92140C;
  padding: 15px 35px;
  text-align: center;
  text-decoration: none;
  display: inline-block;
  font-size: 16px;
  box-shadow: 5px 10px #888888;
}
</style>
